...
# Script to create a zip file of the WP_Petition plugin
# This script should be run from the plugin's root directory

# Set variables
PLUGIN_NAME="wp-petition"
# Read current version using the original method
CURRENT_VERSION=$(grep "Version:" wp-petition.php | awk -F': ' '{print $2}' | tr -d '\r')

if [ -z "$CURRENT_VERSION" ]; then
  echo "Error: Could not find current version number in wp-petition.php"
  exit 1
fi
...
# Update the version in the header line of wp-petition.php using perl
perl -pi -e "s/^(\s*\*\s*Version:\s*)$CURRENT_VERSION/\${1}$NEW_VERSION/" wp-petition.php
if [ $? -ne 0 ]; then
    echo "Error: Failed to update header version in wp-petition.php"
    exit 1
fi

# Update the version in the define line of wp-petition.php using perl
perl -pi -e "s/^(define\('WP_PETITION_VERSION',\s*')$CURRENT_VERSION('\);)/\${1}$NEW_VERSION\${2}/" wp-petition.php
if [ $? -ne 0 ]; then
    echo "Error: Failed to update define version in wp-petition.php"
    exit 1
fi

echo "Updated version in wp-petition.php to $NEW_VERSION"
...
# Important: Copy the MODIFIED wp-petition.php
